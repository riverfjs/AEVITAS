...
// Files are named aevitas-YYYYMMDD.log; a symlink aevitas.log always points to the current file.
func InitLogger(workspace string, debug bool) (*zap.Logger, error) {
	...
	// Pattern: logs/aevitas-20260224.log (new file each day).
	// When the size cap is hit within the same day the timestamp in the suffix
	// gains a time component, e.g. aevitas-20260224T153012.log.
	// logs/aevitas.log is a symlink that always points to the current file.
	rotator, err := rotatelogs.New(
		filepath.Join(logDir, "aevitas-%Y%m%d.log"),
		rotatelogs.WithLinkName(filepath.Join(logDir, "aevitas.log")),
	...
	isDaemon := os.Getenv("AEVITAS_DAEMON") == "1"
